  ...
  return (
    <div className="flex flex-row justify-evenly items-center w-[70%] p-4 text-center min-h-[20rem] h-[20rem]">
        ...
        <div className="h-[100%] flex flex-col justify-evenly items-center w-[50%]">
            <span className="text-3xl text-theme-alt font-medium uppercase">{title}</span>
            <p className="text-justify text-sm">{description?.substring(0,40) + "..."}</p>
            <Link className="p-2 px-6 hover:px-8 bg-[#000] text-theme text-lg transition-all hover:border-theme border-2 border-transparent duration-300" to={currURL + "/" + title}>Read More</Link>
        </div>
    </div>
  )
}
